...
const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ habits, currentDate, onDeleteAll }) => {
  ...
  return (
    <div className="mt-12 md:mt-24 space-y-16 md:space-y-24 animate-in fade-in duration-1000 pb-12">
      {/* Section Header */}
      <div className="flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-6 px-4">
        <div className="flex flex-col md:flex-row items-center gap-4">
          <div className="w-12 h-12 md:w-14 md:h-14 bg-bloom-500 rounded-2xl md:rounded-3xl flex items-center justify-center text-white shadow-xl shadow-bloom-500/20">
      ...
      <div className="space-y-16 px-4">
        ...
        <div className="bg-white rounded-[40px] p-8 md:p-12 border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-10 gap-4">
            <div className="text-center md:text-left">
              <h3 className="text-3xl md:text-5xl font-black text-gray-900 tracking-tighter lowercase">
          ...
          <div className="relative h-[120px] md:h-[200px] w-full flex items-center bg-gray-50/50 rounded-[32px] p-4 border border-gray-100 overflow-hidden">
            <svg viewBox="0 0 1000 150" className="w-full h-full overflow-visible" preserveAspectRatio="none">
              ...
              <path d={ambientPath} fill="none" stroke="url(#lineGradient)" strokeWidth="5" strokeLinecap="round" className="transition-all duration-1000" />
              {heatmapDays.map((data, i) => {
                ...
                return (
                  <circle key={data.day} cx={x} cy={y} r={isToday ? "8" : "4"} className={`${isToday ? 'fill-bloom-600' : 'fill-bloom-400 opacity-30'}`} />
                );
              })}
            </svg>
          </div>
        ...
        {/* Heatmap Section */}
        <div className="bg-white rounded-[40px] p-8 md:p-12 border border-gray-100 shadow-sm flex flex-col xl:flex-row justify-between gap-12">
          <div className="xl:max-w-sm text-center xl:text-left">
            <h3 className="text-2xl md:text-3xl font-black text-gray-900 uppercase tracking-tighter mb-4">Bloom Density</h3>
            <p className="text-sm md:text-base text-gray-500 font-medium leading-relaxed mb-8">Visualization of your total ritual intensity across the entire calendar month.</p>
            <div className="flex items-center justify-center xl:justify-start gap-3 bg-gray-50 px-5 py-3 rounded-full w-fit mx-auto xl:mx-0">
              <span className="text-[10px] font-black text-gray-400 uppercase">Less</span>
              <div className="flex gap-1.5">
                {[1,2,3,4,5].map(i => <div key={i} className={`w-4 h-4 rounded-md ${['bg-gray-100', 'bg-bloom-100', 'bg-bloom-300', 'bg-bloom-500', 'bg-bloom-800'][i-1]}`} />)}
              </div>
              <span className="text-[10px] font-black text-gray-400 uppercase">More</span>
            </div>
          </div>
          
          <div className="flex-1 flex items-center justify-center">
            <div className="grid grid-cols-7 gap-2 md:gap-3 p-4 md:p-6 bg-gray-50/50 rounded-[32px] border border-gray-100 w-full md:w-auto max-w-md">
              {heatmapDays.map((data) => {
                ...
                return (
                  <div key={data.day} className={`aspect-square rounded-lg md:rounded-xl transition-all shadow-sm ${intensityClass} ${isToday ? 'ring-2 ring-bloom-500 ring-offset-2' : ''}`} />
                );
              })}
            </div>
          </div>
        ...
        {/* Danger Zone */}
        <div className="bg-red-50/20 rounded-[40px] p-8 md:p-12 border border-red-100/30">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
            <div>
              <h3 className="text-xl md:text-2xl font-black text-gray-900 uppercase tracking-tighter mb-2">Garden Maintenance</h3>
              <p className="text-sm text-gray-500 font-medium max-w-sm">Reset your rituals? This action permanently clears all your tracking history.</p>
            </div>
            <button 
              onClick={onDeleteAll}
              className="w-full md:w-auto px-10 py-5 bg-white text-red-500 border-2 border-red-100 hover:bg-red-500 hover:text-white hover:border-red-500 rounded-[24px] font-black uppercase tracking-widest text-xs transition-all active:scale-95"
            >
              Clear Entire Garden
            </button>
          </div>
