...
interface HabitModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (name: string, goal: string, color: string, reminderTime?: string, reminderDays?: number[], targetValue?: number, unit?: string) => void;
  onUpdate: (id: string, name: string, goal: string, color: string, reminderTime?: string, reminderDays?: number[], targetValue?: number, unit?: string) => void;
...
const HabitModal: React.FC<HabitModalProps> = ({ isOpen, onClose, onAdd, onUpdate, habitToEdit, onDelete }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (habitToEdit) {
      onUpdate(habitToEdit.id, name, goal, selectedColor, finalReminderTime, finalReminderDays, finalTarget, finalUnit);
    } else {
      onAdd(name, goal, selectedColor, finalReminderTime, finalReminderDays, finalTarget, finalUnit);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm">
      <div className="bg-white rounded-[40px] w-full max-w-md p-10 shadow-2xl animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto no-scrollbar">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={onClose} className="p-2 bg-gray-50 rounded-xl hover:text-red-500 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
             ...
             <input
              ...
              className="w-full px-6 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-bloom-500 outline-none font-bold transition-all"
              value={name}
          ...
          <div className="p-6 bg-gray-50 rounded-3xl space-y-4 border border-gray-100">
            <div className="flex items-center justify-between">
               ...
               <button 
                ...
                className={`w-12 h-6 rounded-full transition-all relative ${isMeasurable ? 'bg-bloom-500' : 'bg-gray-200'}`}
               >
            ...
            {isMeasurable ? (
              <div className="grid grid-cols-2 gap-3 animate-in slide-in-from-top-2 duration-300">
                <div className="space-y-1">
                  ...
                  <input 
                    type="number"
                    min="1"
                    className="w-full px-4 py-3 rounded-xl bg-white border border-gray-100 font-bold outline-none focus:border-bloom-500"
                    value={targetValue}
              ...
              <input
                ...
                className="w-full px-6 py-4 rounded-2xl bg-white border border-gray-100 outline-none font-bold text-sm"
                value={goal}
          ...
          <div className="p-6 bg-bloom-50/50 rounded-3xl border border-bloom-100 space-y-4">
            <div className="flex items-center justify-between">
               ...
               <button 
                ...
                className={`w-12 h-6 rounded-full transition-all relative ${enableReminder ? 'bg-bloom-500' : 'bg-gray-200'}`}
               >
              ...
              <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                <div className="flex items-center justify-between bg-white px-4 py-3 rounded-2xl border border-bloom-100">
                  ...
                  <input 
                    ...
                    className="bg-transparent font-black text-bloom-600 outline-none"
                  />
                </div>
          ...
          <button type="submit" className="w-full py-5 bg-gray-900 text-white rounded-3xl font-black uppercase tracking-widest shadow-xl hover:scale-[1.02] active:scale-95 transition-all mt-4">
            {habitToEdit ? 'Update Garden' : 'Plant Habit'}
          </button>

          {habitToEdit && onDelete && (
            <button type="button" onClick={onDelete} className="w-full py-3 text-red-400 font-bold text-xs uppercase tracking-widest hover:text-red-600 transition-colors">
              Uproot Ritual
            </button>
          )}
        </form>
