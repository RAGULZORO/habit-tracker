...
const MonthlyGrid: React.FC<MonthlyGridProps> = ({ habits, currentDate, onToggle, onDeleteHabit, onEditHabit, onViewDetail }) => {
  ...
  return (
    <div className="bg-white rounded-[24px] md:rounded-[40px] border border-gray-100 shadow-sm overflow-hidden">
      {/* Horizontal scroll container with custom scrollbar hiding */}
      <div className="overflow-x-auto no-scrollbar snap-x snap-mandatory">
        <table className="w-full border-separate border-spacing-0">
          <thead>
            <tr className="bg-white">
              {/* Sticky first column header */}
              <th className="sticky left-0 z-40 bg-white/95 backdrop-blur-md p-4 md:p-8 text-left border-b border-gray-100 min-w-[140px] md:min-w-[280px] shadow-[4px_0_10px_-5px_rgba(0,0,0,0.05)]">
                <div className="flex flex-col">
                  <span className="text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-[0.2em]">Habit Tracker</span>
              ...
              {daysArray.map((day) => {
                ...
                return (
                  <th key={day} className={`p-2 md:p-4 min-w-[50px] md:min-w-[64px] border-b border-gray-100 snap-center ${isToday ? 'bg-bloom-50/50' : ''}`}>
                    <div className="flex flex-col items-center justify-center py-1">
                      <span className={`text-[9px] md:text-[10px] font-black uppercase mb-1 ${isToday ? 'text-bloom-600' : 'text-gray-300'}`}>{dayName}</span>
                ...
                <tr key={habit.id} className="group hover:bg-gray-50/30 transition-colors">
                  ...
                  <td className="sticky left-0 z-30 bg-white p-3 md:p-6 border-b border-gray-50 shadow-[4px_0_10px_-5px_rgba(0,0,0,0.05)] group-hover:bg-gray-50 transition-colors">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      >
                        <span className="text-sm md:text-base font-bold text-gray-900 tracking-tight group-hover:text-bloom-600 transition-colors truncate">
                          {habit.name}
                        </span>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <span className="text-[10px] font-bold text-gray-300 uppercase tracking-tighter truncate max-w-[80px]">
                            {habit.goal || 'No goal'}
                          </span>
                        </div>
                      </div>
                      
                      {/* Tappable Edit/Delete Icons for Mobile (larger targets) */}
                      <div className="flex items-center gap-1 shrink-0 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                        <button 
                          ...
                          className="w-8 h-8 flex items-center justify-center text-gray-300 hover:text-bloom-600 bg-gray-50 md:bg-transparent rounded-lg"
                        >
                  ...
                  {/* Grid cells with large tap targets */}
                  {daysArray.map((day) => {
                    const dateStr = formatDateForGrid(year, month, day);
                        ...
                        <button
                          ...
                          className={`w-10 h-10 md:w-11 md:h-11 rounded-xl md:rounded-2xl mx-auto transition-all duration-300 flex items-center justify-center border-2 ${
                            isDone 
                              ...
                              : `bg-gray-50/50 border-transparent ${isToday ? 'hover:border-bloom-200 cursor-pointer active:scale-90 ring-2 ring-bloom-500/10' : 'cursor-default opacity-40'}`
                          }`}
                        >
                          {isDone ? (
      ...
      <div className="md:hidden p-3 bg-gray-50/80 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">
